...
%% General comment:
%% - quelques problemes de notations: w_j^{(k)} et w_j^{(t)} ont differentes significations
%% - 3.1/3.2 en faire des paragraphes ?
%% - J'ai mergé les deux paragraphes sur le boosting dans la section 3.
%% - attention à significantly
%% - Forest's shape, model's predictions
%% - Table 3: attention couleur BD MUSK2.
...
GIF's learning phase is described in Algorithm \ref{alg:gif}.  Starting from a
constant model (step 5), it builds an additive model in the form of Equation
...
candidate nodes $C_t$ is built uniformly at random from the total candidate list
$C$ (step 8). The node $j^*$ among those of $C_t$ which contributes the most to
a decrease of the global loss is selected (step 9) and introduced in the model
via its indicator function $z_{j^*}$ and its optimal weight $w^*_j$ tempered by
some learning rate $\lambda$ (step 10). This node is then split locally
according to the reference tree growing strategy $\mathcal{A}$ (step 11) and
replaced by its two children in the candidate list (step 12). The process is
stopped when the node budget $B$ is reached.
...
\paragraph{Regularization.} \pgnote{Remove this to gain space?}
...
node indicator functions and would be expanded at each
iteration. Overall, GIF leads however to very different models than
standard tree boosting approaches. These latter indeed usually
exploit depth-constrainted decision trees and these trees are
furthermore optimized globally. Notice also that the weights in GIF
can be multidimensional to accommodate for multiclass or multioutput
problems, whereas they are usually scalar in Boosting (with
  potentially multioutput base learners).
...
%UCI: Abalone, CT Slice, Cadata
%TODO hwang F5

\paragraph{Datasets.} \pgnote{Dans l'appendice avec la table?}
...
with different learning sample/testing sample splits (LS and TS sizes
are in Table \ref{tab:datasets}).
...
are examined for each split, in both classification and regression. Note that
GIF is implemented on top of the Scikit-Learn library.
...
exponential loss with a saturation $\theta = 3$.
...
seems to work best. At $1\%$, it loses only on the binarized version of Vowel and 
...
with a best-first local heuristic, were considered but clearly work against the
ensemble averaging idea. For instance, a best-first approach could be to
develop a forest by expanding at each step the node which reduces the local
impurity the most. For instance, this would result---at $1\%$ under the same
protocol---in errors of $15.4 \pm 0.5$ for Friedman1 and $31.4 \pm 18.0$ for
Ringnorm, which are much worse than with ET$_{1\%}$.
...
budget maxes out at $59900$ nodes, corresponding to $10\%$). A unit learning rate 
...
forest. Although motivated as a pre-pruning technique for Random forests, the
