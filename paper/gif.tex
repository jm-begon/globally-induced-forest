...
In all cases, the experiments were repeated ten times with different learning 
sample/testing sample splits. The results are reported in Tables \ref{tab:reg} 
and \ref{tab:cls}.

  
\begin{table*}[t]
\caption{Average mean square error for ET and GIF 
...
\begin{table*}[t]
\caption{Average error rate ($\%$) for ET and GIF for $1\%$ and $10\%$ budgets
  over ten folds ($m=\sqrt{p}$, $\lambda=10^{-1.5}$, $T=1000$,
  $CW=1$). GIF$_{SQ,\cdot}$ relates to the multi-output square loss. GIF$_{TE,
    \cdot}$ relates to the trimmed exponential loss with $\theta=3$.}
\label{tab:cls}
\begin{center}
\begin{small}
\begin{sc}
...
error rate, in percent, for Twonorm and Musk2) and fitting time 
...
\begin{table}[t]
\caption{Average error rate (in percent) over ten folds for the 
...
(error rate, in percent, with the trimmed exponential loss and 
...
error metric is the error rate (in percent). $\lambda=10^{-1.5}$, 
depth$=1$.}
\label{tab:boosting}
\begin{center}
\begin{small}
...
exponential loss with $\theta=3$, $T=1000$, $m=\sqrt{p}$,$\lambda=10^{-1.5}$, 
