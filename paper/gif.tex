...
This mix of global, local, and greedy optimization results in a
fast pre-pruning approach to build lightweight, yet accurate forests learned on
the whole training set. Contrary to post-pruning approaches, GIFs circumvent
the need to build the whole forest first, resulting in a fast learning
algorithm and discarding the need for a large temporary storage.
...
\cite{friedman2001gradboost}, where however the set of base learners would be composed
...
list. Finally, we would like also to explore further the comparison between GIF
and boosting methods, in particular \cite{johnson2014regforest}'s regularized
