...
GIFs will build such a model, by progressively inserting new nodes and their 
indicator functions in the model. The weight corresponding to the new node is 
optimized according to some loss function $L: \mathcal{Y} \times \mathcal{Y} 
\rightarrow \mathbb{R}$ expressing the degree to which the model predictions 
disagree with the ground truth. As a consequence, each node in the final model 
has a non-zero weight $w_j$.
Note however that, as soon as an internal node and both its successors are 
selected, it can be removed from the sum by pushing its weight to its 
successors.
...
last three are multiclass. The three in the middle are there binary versions.}
\vskip -1em
\label{tab:cls}
\begin{center}
\begin{footnotesize}
